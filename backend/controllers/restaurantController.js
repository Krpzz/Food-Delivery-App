...
// ---------- Public browsing (Section 18: search, filter, sort) ----------
...
// "price" / "deliveryFee" / "popularity" sort will be meaningful once
// Checkout (Step 7) and Orders (Step 8) exist to generate that data.
...
const getRestaurantById = async (req, res, next) => {
  try {
    // Deliberately does NOT show unapproved restaurants, even to a logged-in
    // owner — owners view their own (possibly pending) restaurants through
    // getMyRestaurants instead, keeping this endpoint simple and truly public.
...
// ---------- Owner-scoped (protected) ----------

// One owner can run several restaurants (the seed data uses 3 owners across
// 10 restaurants), so this returns an array rather than a single document.
...
const createRestaurant = async (req, res, next) => {
  try {
    ...
    const restaurant = await Restaurant.create({
      ...
      logo,
      coverImage,
      // isApproved defaults to false — only admin tooling (Step 11) flips it.
    });
...
const updateRestaurant = async (req, res, next) => {
  try {
    ...
    // isApproved is intentionally excluded from this list — an owner can
    // never self-approve their own restaurant (Section 20 / Section 5).
...
const deleteRestaurant = async (req, res, next) => {
  try {
    ...
    // Cascade delete so menu items never end up orphaned, pointing at a
